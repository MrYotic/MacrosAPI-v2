...
namespace MacrosAPI_v2
{
    public abstract class Macros
    {
        ...
        public void SetHandler(MacrosManager handler) => _handler = handler;
        ...
        {
            Handler.UnLoadMacros(macros);
            if (Handler.OnUnloadPlugin != null) Handler.OnUnloadPlugin();
        }
        protected void UnLoadPlugin() => UnLoadPlugin(this);
        protected void RunScript(FileInfo filename) => Handler.LoadMacros(new Script(filename));
        #endregion
        #region Ивенты плагина
        public virtual void Initialize() { }
        public virtual void Update() { }
        public virtual void ReceivedObject(object s) { }
        public virtual bool OnKeyDown(Key key, bool repeat) => false;
        public virtual bool OnKeyUp(Key key) => false;
        public virtual bool OnMouseDown(MouseKey key) => false;
        public virtual bool OnMouseUp(MouseKey key) => false;
        public virtual bool OnMouseMove(int x, int y) => false;
        public virtual bool OnMouseWheel(int rolling) => false;
        #endregion
        #region Методы плагина
        protected void Sleep(int delay) => Thread.Sleep(delay);
        #region Работа с клавиатурой
        protected bool IsKeyPressed(Keys key) => GetAsyncKeyState((int) key) != 0;
        ...
        protected bool IsKeyDown(Key key) => IsKeyDown(Handler.keyboardDeviceID, key);
        protected bool IsKeyUp(int deviceID, Key key) => !IsKeyDown(deviceID, key);
        protected bool IsKeyUp(Key key) => IsKeyUp(Handler.keyboardDeviceID, key);
        ...
        protected void KeyDown(params Key[] keys) => KeyDown(Handler.keyboardDeviceID, keys);
        ...
        protected void KeyUp(params Key[] keys) => KeyUp(Handler.keyboardDeviceID, keys);
        #endregion
        ...
        protected void MouseScroll(short rolling) => MouseScroll(Handler.mouseDeviceID, rolling);
        ...
        protected void MouseMove(int x, int y) => MouseMove(Handler.mouseDeviceID, x, y);
        protected void MouseSet(int x, int y) => MouseSet(Handler.mouseDeviceID, x, y);
        protected void MouseDown(params MouseKey[] keys) => MouseDown(Handler.mouseDeviceID, keys);
        protected void MouseUp(params MouseKey[] keys) => MouseUp(Handler.mouseDeviceID, keys);
        #endregion
        ...
        {
            var pid = 0;
            WinAPI.GetWindowThreadProcessId(WinAPI.GetForegroundWindow(), ref pid);
            return Process.GetProcessById(pid);
        }
        protected void PluginPostObject(object obj) => Handler.OnMacrosPostObjectMethod(obj);
        #endregion
    }
}